...
describe('/api/settings', () => {
  ...
  const mockSettingsStorage = vi.mocked(require('../lib/settings-storage').getSettingsStorage);
  ...
  it('handles GET request successfully', async () => {
    ...
    const mockStorage = mockSettingsStorage();
    mockStorage.getMaskedSettings.mockResolvedValue(mockSettings);
  ...
  it('handles POST request with valid data', async () => {
    ...
    mockStorage.getMaskedSettings.mockResolvedValue({
      anthropicKey: 'sk-ant-••••••••',
  ...
  it('handles DELETE request', async () => {
    ...
    const mockStorage = mockSettingsStorage();
    mockStorage.deleteSetting.mockResolvedValue(undefined);
  ...
  it('handles storage errors gracefully', async () => {
    ...
    const mockStorage = mockSettingsStorage();
    mockStorage.getMaskedSettings.mockRejectedValue(new Error('Database error'));
...
describe('/api/setup-status', () => {
  ...
  it('returns setup status successfully', async () => {
    const mockStorage = mockSettingsStorage();
    mockStorage.isSetup.mockResolvedValue(false);
    mockStorage.getMaskedSettings.mockResolvedValue({
      configured: {
        anthropic: false,
        openai: false,
  ...
  it('returns completed setup status', async () => {
    const mockStorage = mockSettingsStorage();
    mockStorage.isSetup.mockResolvedValue(true);
    mockStorage.getMaskedSettings.mockResolvedValue({
      configured: {
        anthropic: true,
        openai: true,
  ...
  it('handles storage errors gracefully', async () => {
    const mockStorage = mockSettingsStorage();
    mockStorage.isSetup.mockRejectedValue(new Error('Database error'));
